...
{
    if (index >= ecs_vector_count(chunked->sparse)) {
        ecs_sparse_set_size(chunked, index + 1);
...
void ecs_sparse_grow(
    ...
    uint32_t count)
{
    uint32_t current = ecs_sparse_count(chunked);

    while (ecs_sparse_size(chunked) <= (count + current)) {
        add_chunk(chunked);
    }
}
